...
function deploy_single_local () {
  ...
  cd $bin_dir
  ansible-playbook $home_dir"/site.yml" --extra-vars "home_dir=$home_dir/ deploy_env=local_single"
}
...
#  sh ../lib/generate_vagrant_ssh_config $home_dir"/hosts"
  ...
  cd $bin_dir
  ansible-playbook $home_dir"/site.yml" --extra-vars "home_dir=$home_dir/ deploy_env=local_multi"
}
